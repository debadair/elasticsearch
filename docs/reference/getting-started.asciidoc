...
. <<getting-started-install, Get an {es} cluster up and running>>
. <<getting-started-index, Index some sample documents>>
. <<getting-started-search, Search for documents>> using the {es} query language
. <<getting-started-aggregations, Analyze the results>> using bucket and metrics aggregations
