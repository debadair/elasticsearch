...
[discrete]
=== Template and settings priority

If a new data stream or index matches more than one index template, 
the index template with the highest priority value is used.

If an index is created with explicit settings and also matches an index template, 
the settings from the create index request take precedence over settings specified in the 
template and its component templates.

When a composable template matches a given index it always takes precedence over a legacy template. 
If no composable template matches, a <<indices-templates-v1, legacy template>> may still match and be applied.
