...
++++
<titleabbrev>Snapshot retention</titleabbrev>
++++
...
| `slm.retention_schedule` | `0 30 1 * * ?` | A periodic or absolute time schedule for when
  ...
  <<slm-api-execute-retention>>. Defaults to daily at 1:30am in the master
  node's timezone.
...
been taken without using the <<slm-api-execute-lifecycle>>, they are treated as not
