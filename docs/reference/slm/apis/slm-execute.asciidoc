...
[[slm-execute-lifecycle-request]]
==== {api-request-title}

`PUT /_slm/policy/<snapshot-lifecycle-policy-id>/_execute`


[[slm-execute-lifecycle-desc]]
...
[[slm-execute-lifecycle-path-params]]
...
To see the status of a policy's most recent snapshot, you can use the <<slm-get, get SLM policy API>>.